...
export function PersonaDetail({ live }: { live: PersonaLive }) {
  ...
  return (
    <section className="min-w-0 rounded-lg border border-slate-200 bg-white shadow-[0_12px_30px_rgba(15,23,42,0.04)]">
      <header className="flex min-w-0 items-start justify-between gap-4 border-b border-slate-200 p-5">
        <div className="min-w-0">
          <p className="text-[11px] font-semibold uppercase tracking-[0.12em] text-slate-500">
            Evidence trail
          </p>
          <h3 className="mt-2 text-base font-semibold text-slate-950">{persona.name}</h3>
          <p className="mt-1 break-words text-xs leading-5 text-slate-500">{persona.role}</p>
        </div>
        {outcome && (
      ...
      {latestScreenshot && (
        <div className="min-w-0 border-b border-slate-200 bg-slate-50 p-4">
          <Image
      ...
      <div className="min-w-0 max-h-96 overflow-y-auto p-5">
        <p className="text-xs font-semibold uppercase tracking-[0.12em] text-slate-500">
          Full journey
        </p>
...
function StepRow({ step }: { step: PersonaLive["steps"][number] }) {
  ...
  return (
    <li className="min-w-0 border-l border-slate-200 pl-4">
